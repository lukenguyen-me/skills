...
related install documentation. Nine skills live under `skills/<name>/` in a
flat layout.
...
- `.codex-plugin/plugin.json`: Codex plugin manifest listing all nine skill paths
...
| `pr-feedback-loop` | `skills/pr-feedback-loop/` | Codex app PR review feedback automation |
...
Repo-owned skills (the nine in `skills/`) appear as one-hop symlinks:
